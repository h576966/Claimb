...
public class UserSession: ObservableObject {
    ...
    @Published public var selectedPrimaryRole: String = "TOP"
        ...
        Task {
            do {
                ...
                if let summoner = allSummoners.first(where: { 
                    ...
                    print("🔐 [UserSession] Found existing summoner in database: \(summoner.gameName)#\(summoner.tagLine)")
                    await MainActor.run {
                    ...
                    print("🔐 [UserSession] Summoner not found in database, recreating from stored credentials")
                    try await recreateSummonerFromCredentials(gameName: gameName, tagLine: tagLine, region: region)
                }
            } catch {
    ...
    private func recreateSummonerFromCredentials(gameName: String, tagLine: String, region: String) async throws {
        let dataManager = DataManager(
            modelContext: modelContext,
            riotClient: RiotHTTPClient(apiKey: APIKeyManager.riotAPIKey),
            dataDragonService: DataDragonService()
    ...
    public func login(summoner: Summoner) {
        ...
        print("🔐 [UserSession] Credentials saved - gameName: \(savedGameName ?? "nil"), tagLine: \(savedTagLine ?? "nil"), region: \(savedRegion ?? "nil")")
