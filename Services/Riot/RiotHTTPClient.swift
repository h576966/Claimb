...
public class RiotHTTPClient: RiotClient {
    ...
    private func buildAccountURL(gameName: String, tagLine: String, region: String) throws -> URL {
        let encodedGameName =
            gameName.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? gameName
        let encodedTagLine =
            tagLine.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? tagLine

        // Account-v1 uses different regional endpoints: europe, americas, asia
        let accountEndpoint = convertToAccountEndpoint(region)
        let urlString =
            "https://\(accountEndpoint).api.riotgames.com/riot/account/v1/accounts/by-riot-id/\(encodedGameName)/\(encodedTagLine)"
    ...
    private func buildSummonerURL(puuid: String, region: String) throws -> URL {
        ...
        let urlString =
            "https://\(regionalEndpoint).api.riotgames.com/lol/summoner/v4/summoners/by-puuid/\(puuid)"
        guard let url = URL(string: urlString) else {
            throw RiotAPIError.invalidURL
        }
        return url
    ...
    private func buildMatchHistoryURL(puuid: String, region: String, count: Int) throws -> URL {
        ...
        let urlString =
            "https://\(matchEndpoint).api.riotgames.com/lol/match/v5/matches/by-puuid/\(puuid)/ids?count=\(count)"
        guard let url = URL(string: urlString) else {
            throw RiotAPIError.invalidURL
        }
        return url
    ...
    private func convertToRegionalEndpoint(_ region: String) -> String {
        ...
        default:
            ClaimbLogger.warning(
                "Unknown region, using default", service: "RiotAPI",
                metadata: [
                    "region": region,
                    "default": "na1",
                ])
    ...
    private func convertToAccountEndpoint(_ region: String) -> String {
        ...
        default:
            ClaimbLogger.warning(
                "Unknown region for account endpoint, using default", service: "RiotAPI",
                metadata: [
                    "region": region,
                    "default": "americas",
                ])
