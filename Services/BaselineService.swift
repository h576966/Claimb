...
public struct PerformanceAnalysis {
    ...
    public init(metric: String, playerValue: Double, baseline: Baseline, performance: Baseline.PerformanceLevel, message: String) {
        self.metric = metric
...
public struct RolePerformanceAnalysis {
    ...
    public init(role: String, championClass: String, analyses: [PerformanceAnalysis], overallScore: Double, summary: String) {
        self.role = role
...
public class BaselineService {
    ...
    private func getRoleSpecificKPIs(for role: String) -> [String] {
        switch role {
        case "TOP":
            return ["deaths_per_game", "kill_participation_pct", "vision_score_per_min", "damage_taken_share_pct", "cs_per_min"]
        case "JUNGLE":
            return ["deaths_per_game", "kill_participation_pct", "vision_score_per_min", "objective_participation_pct", "cs_per_min"]
        case "MIDDLE":
            return ["deaths_per_game", "kill_participation_pct", "vision_score_per_min", "team_damage_pct", "cs_per_min"]
        case "BOTTOM":
            return ["deaths_per_game", "kill_participation_pct", "vision_score_per_min", "cs_per_min", "team_damage_pct"]
        case "UTILITY":
            return ["deaths_per_game", "kill_participation_pct", "vision_score_per_min", "objective_participation_pct"]
        default:
    ...
    private func analyzeKPI(
        ...
        guard let baseline = try await getBaseline(
            role: role,
        ...
        let message = generatePerformanceMessage(kpi: kpi, value: playerValue, performance: performance)
    ...
    private func calculateKPIValue(kpi: String, participant: Participant, match: Match) -> Double {
        ...
        case "vision_score_per_min":
            return matchDurationMinutes > 0 ? Double(participant.visionScore) / matchDurationMinutes : 0.0
        ...
        case "objective_participation_pct":
            let totalParticipated = participant.dragonTakedowns + participant.riftHeraldTakedowns + 
                                  participant.baronTakedowns + participant.hordeTakedowns + participant.atakhanTakedowns
            let totalTeamObjectives = match.getTeamObjectives(teamId: participant.teamId)
            return totalTeamObjectives > 0 ? Double(totalParticipated) / Double(totalTeamObjectives) : 0.0
            
        case "team_damage_pct":
            let teamTotalDamage = match.participants.reduce(0) { $0 + $1.totalDamageDealt }
            return teamTotalDamage > 0 ? Double(participant.totalDamageDealt) / Double(teamTotalDamage) : 0.0
            
        case "damage_taken_share_pct":
            let teamTotalDamageTaken = match.participants.reduce(0) { $0 + $1.totalDamageTaken }
            return teamTotalDamageTaken > 0 ? Double(participant.totalDamageTaken) / Double(teamTotalDamageTaken) : 0.0
            
        case "gold_per_min":
            return matchDurationMinutes > 0 ? Double(participant.goldEarned) / matchDurationMinutes : 0.0
