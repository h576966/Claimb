...
struct PerformanceView: View {
    ...
    private var performanceOverviewCard: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.md) {
            ...
            let roleMatches = matches.filter { match in
                match.participants.contains { $0.puuid == summoner.puuid && RoleUtils.normalizeRole($0.role) == selectedRole }
            }
            
            let wins = roleMatches.compactMap { match in
                match.participants.first(where: { $0.puuid == summoner.puuid })?.win
            }.filter { $0 }.count
            
            let winRate = roleMatches.isEmpty ? 0.0 : Double(wins) / Double(roleMatches.count)
            
            HStack {
                VStack(alignment: .leading, spacing: DesignSystem.Spacing.xs) {
                    Text("\(roleMatches.count) Games")
                        .font(DesignSystem.Typography.bodyBold)
                        .foregroundColor(DesignSystem.Colors.textPrimary)
                    
                    Text("in \(RoleUtils.displayName(for: selectedRole))")
                        .font(DesignSystem.Typography.caption)
                        .foregroundColor(DesignSystem.Colors.textSecondary)
                }
                ...
                VStack(alignment: .trailing, spacing: DesignSystem.Spacing.xs) {
                    Text("\(Int(winRate * 100))%")
                        ...
                        .fontWeight(.bold)
                        .foregroundColor(winRateColor(winRate))
    ...
    private func calculateRoleKPIs(
        ...
        let killParticipation = participants.map { participant in
            ...
            return teamKills > 0 ? Double(participant.kills + participant.assists) / Double(teamKills) : 0.0
        ...
        if role == "JUNGLE" || role == "SUPPORT" {
            let objectiveParticipation = participants.map { participant in
                let match = matches.first { $0.participants.contains(participant) }
        ...
        if role == "TOP" {
            let damageTakenShare = participants.map { participant in
                let match = matches.first { $0.participants.contains(participant) }
                let teamDamageTaken = match?.participants.reduce(0) { $0 + $1.totalDamageTaken } ?? 0
