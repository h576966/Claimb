...
public struct ClaimbLogger {
    ...
    public static func debug(
        _ message: String, service: String = "App", metadata: [String: String] = [:]
    ) {
        #if DEBUG
            let metadataString =
                metadata.isEmpty
    ...
    public static func error(
        _ message: String, service: String = "App", error: Error? = nil,
        ...
        let metadataString =
            allMetadata.isEmpty
            ? "" : " | \(allMetadata.map { "\($0.key)=\($0.value)" }.joined(separator: ", "))"
    ...
    public static func apiRequest(
        _ endpoint: String, method: String = "GET", service: String = "API"
    ) {
        debug(
            "\(method) \(endpoint)", service: service,
            metadata: [
                "endpoint": endpoint,
                "method": method,
            ])
    ...
    /// Log data operations
    public static func dataOperation(
        _ operation: String, count: Int? = nil, service: String = "DataManager"
    ) {
        ...
        info(
            "\(operation)\(count != nil ? " (\(count!) items)" : "")", service: service,
            metadata: metadata)
    }
    ...
    {
        debug(
            "\(metric): \(String(format: "%.2f", value))", service: service,
            metadata: [
                "metric": metric,
    ...
    public static func cache(_ operation: String, key: String? = nil, service: String = "Cache") {
        ...
        debug(
            "Cache \(operation)\(key != nil ? " for \(key!)" : "")", service: service,
            metadata: metadata)
    }
}
