...
public class DataCoordinator {
    ...
    public init(modelContext: ModelContext) {
        let riotClient = RiotHTTPClient(apiKey: APIKeyManager.riotAPIKey)
    ...
    public func loadMatches(for summoner: Summoner, limit: Int = 50) async -> UIState<[Match]> {
        ClaimbLogger.info("Loading matches", service: "DataCoordinator", metadata: [
        ...
        do {
            ...
            if existingMatches.isEmpty {
                ...
                ClaimbLogger.info("No existing matches, loading initial batch", service: "DataCoordinator")
                try await dataManager.loadInitialMatches(for: summoner)
            } else {
                // Load any new matches incrementally
                ClaimbLogger.info("Found existing matches, checking for new ones", service: "DataCoordinator", metadata: [
    ...
    public func refreshMatches(for summoner: Summoner) async -> UIState<[Match]> {
        ClaimbLogger.info("Refreshing matches", service: "DataCoordinator", metadata: [
            "summoner": summoner.gameName
        ])
        ...
        do {
            ...
            ClaimbLogger.error("Failed to refresh matches", service: "DataCoordinator", error: error)
            return .error(error)
        }
    }
    ...
    public func createOrUpdateSummoner(gameName: String, tagLine: String, region: String) async -> UIState<Summoner> {
        ClaimbLogger.info("Creating/updating summoner", service: "DataCoordinator", metadata: [
            "gameName": gameName,
            "tagLine": tagLine,
            "region": region
        ])
        ...
        do {
            ...
            ClaimbLogger.error("Failed to create/update summoner", service: "DataCoordinator", error: error)
            return .error(error)
        }
    }
